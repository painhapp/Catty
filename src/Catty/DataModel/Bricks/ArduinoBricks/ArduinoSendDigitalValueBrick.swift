...
class ArduinoSendDigitalValueBrick: ArduinoBrick, BrickFormulaProtocol {
    var pin: Formula?
    var value: Formula?
    
    func brickTitle() -> String? {
        return kLocalizedArduinoSetDigitalValue + ("%@\n") + (kLocalizedArduinoSetPinValueTo) + ("%@")
    }
    ...
    override func isEqual(to brick: Brick?) -> Bool {
        if !(pin?.isEqual(to: (brick as? ArduinoSendDigitalValueBrick)?.pin) ?? false) {
            return false
        }
        if !(value?.isEqual(to: (brick as? ArduinoSendDigitalValueBrick)?.value) ?? false) {
            return false
        }
        return true
    }
