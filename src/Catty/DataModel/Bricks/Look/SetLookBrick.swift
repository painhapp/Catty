...
class SetLookBrick: Brick, BrickLookProtocol {
    var look: Look?
    ...
    func brickTitle() -> String? {
        return kLocalizedSetLook + ("\n%@")
    }
    
    // MARK: - Description
    
    override func description() -> String {
        return "SetLookBrick (Look: \(look?.name ?? ""))"
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if look?.isEqual(to: (brick as? SetLookBrick)?.look) ?? false {
            return true
        }
        return false
    }
