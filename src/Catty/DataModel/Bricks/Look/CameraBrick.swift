/**
 *  Copyright (C) 2010-2018 The Catrobat Team
 *  (http://developer.catrobat.org/credits)
 *
 *  This program is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License as
 *  published by the Free Software Foundation, either version 3 of the
 *  License, or (at your option) any later version.
 *
 *  An additional term exception under section 7 of the GNU Affero
 *  General Public License, version 3, is available at
 *  (http://developer.catrobat.org/license_additional_term)
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with this program.  If not, see http://www.gnu.org/licenses/.
 */

class CameraBrick: Brick, BrickStaticChoiceProtocol {
    var cameraChoice: Int = 0
    
    init(choice: Int) {
        super.init()
        
        cameraChoice = choice
        
    }
    
    func isEnabled() -> Bool {
        return cameraChoice == 1
    }
    
    func brickTitle() -> String? {
        return kLocalizedCamera + ("\n%@")
    }
    
    override func setDefaultValuesFor(_ spriteObject: SpriteObject?) {
        cameraChoice = 1
    }
    
    func setChoice(_ choice: String?, forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) {
        if (choice == kLocalizedOff) {
            cameraChoice = 0
        } else {
            cameraChoice = 1
        }
    }
    
    func choice(forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) -> String? {
        let choices = possibleChoices(forLineNumber: 1, andParameterNumber: 0)
        return choices?[cameraChoice]
    }
    
    func possibleChoices(forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) -> [String]? {
        let choices = [kLocalizedOff, kLocalizedOn] as? [String]
        return choices
    }
    
    // MARK: - Description
    
    override func description() -> String {
        return String(format: "camera choice (%i)", cameraChoice)
    }
    
    // MARK: - Resources
    
    override func getRequiredResources() -> Int {
        return ResourceType.noResources
    }
    
    // MARK: - Resources
}
