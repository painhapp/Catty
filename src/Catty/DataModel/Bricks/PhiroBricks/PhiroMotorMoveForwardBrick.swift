...
class PhiroMotorMoveForwardBrick: PhiroBrick, BrickPhiroMotorProtocol, BrickFormulaProtocol {
    var motor = ""
    var formula: Formula?
    ...
    override func description() -> String {
        return String(format: "Move Phiro Motor Forward (Motor: %lu)", UInt(motor)!)
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if motor != (brick as? PhiroMotorMoveForwardBrick)?.motor {
            return false
        }
        if !(formula?.isEqual(to: (brick as? PhiroMotorMoveForwardBrick)?.formula) ?? false) {
            return false
        }
        return true
    }
    ...
    override func getRequiredResources() -> Int {
        return ResourceType.bluetoothPhiro | (formula?.getRequiredResources() ?? 0)
    }
}
