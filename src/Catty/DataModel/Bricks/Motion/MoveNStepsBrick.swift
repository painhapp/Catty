...
class MoveNStepsBrick: Brick, BrickFormulaProtocol {
    var steps: Formula?
    ...
    override func setDefaultValuesFor(_ spriteObject: SpriteObject?) {
        steps = Formula(integer: 10)
    }
    
    func brickTitle() -> String? {
        let localizedStep = steps?.isSingularNumber() ?? false ? kLocalizedStep : kLocalizedSteps
        return kLocalizedMove + ("%@ " + (localizedStep))
    }
