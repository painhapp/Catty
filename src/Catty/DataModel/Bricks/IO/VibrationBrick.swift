...
class VibrationBrick: Brick, BrickFormulaProtocol {
    ...
    func brickTitle() -> String? {
        let durationStr = durationInSeconds?.isSingularNumber() ?? false ? kLocalizedSecond : kLocalizedSeconds
        return kLocalizedVibrateFor + ("%@ " + (durationStr))
    }
    ...
    override func getRequiredResources() -> Int {
        return ResourceType.vibration | (durationInSeconds?.getRequiredResources() ?? 0)
    }
}
