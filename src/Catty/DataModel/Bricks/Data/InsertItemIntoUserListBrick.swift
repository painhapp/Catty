...
class InsertItemIntoUserListBrick: Brick, BrickFormulaProtocol, BrickListProtocol {
    ...
    override func description() -> String {
        if let aList = userList {
            return "InsertItemIntoUserListBrick (Userlist: \(aList))"
        }
        return ""
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if !(userList?.isEqual(to: (brick as? InsertItemIntoUserListBrick)?.userList) ?? false) {
            return false
        }
        if !(elementFormula?.isEqual(to: (brick as? InsertItemIntoUserListBrick)?.elementFormula) ?? false) {
            return false
        }
        if !(index?.isEqual(to: (brick as? InsertItemIntoUserListBrick)?.index) ?? false) {
            return false
        }
        return true
    }
