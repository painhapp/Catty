...
class DeleteItemOfUserListBrick: Brick, BrickFormulaProtocol, BrickListProtocol {
    ...
    override func description() -> String {
        if let aList = userList {
            return "Delete Item From User List Brick: Userlist: \(aList)"
        }
        return ""
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if !(userList?.isEqual(to: (brick as? DeleteItemOfUserListBrick)?.userList) ?? false) {
            return false
        }
        if !(listFormula?.isEqual(to: (brick as? DeleteItemOfUserListBrick)?.listFormula) ?? false) {
            return false
        }
        return true
    }
