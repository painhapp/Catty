...
class HideTextBrick: Brick, BrickVariableProtocol {
    ...
    override func description() -> String {
        if let aVariable = userVariable {
            return "Hide Variable Brick: Uservariable: \(aVariable)"
        }
        return ""
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if !(userVariable?.isEqual(to: (brick as? HideTextBrick)?.userVariable) ?? false) {
            return false
        }
        return true
    }
