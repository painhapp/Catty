...
class SetVariableBrick: Brick, BrickFormulaProtocol, BrickVariableProtocol {
    ...
    override func description() -> String {
        if let aVariable = userVariable {
            return "SetVariableBrick (Uservariable: \(aVariable))"
        }
        return ""
    }
    
    override func isEqual(to brick: Brick?) -> Bool {
        if !(userVariable?.isEqual(to: (brick as? SetVariableBrick)?.userVariable) ?? false) {
            return false
        }
        if !(variableFormula?.isEqual(to: (brick as? SetVariableBrick)?.variableFormula) ?? false) {
            return false
        }
        return true
    }
