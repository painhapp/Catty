...
class BrickCellPhiroMotorData: iOSCombobox, BrickCellDataProtocol, iOSComboboxDelegate {
    ...
    required init(frame: CGRect, andBrickCell brickCell: BrickCell?, andLineNumber line: Int, andParameterNumber parameter: Int) {
        ...
        if brickCell?.scriptOrBrick is BrickPhiroMotorProtocol != nil {
            let motorBrick = brickCell?.scriptOrBrick as? (Brick & BrickPhiroMotorProtocol)
            let currentMotor = motorBrick?.motor(forLineNumber: line, andParameterNumber: parameter)
            switch PhiroHelper.string(toMotor: currentMotor) {
            case Motor.both:
        ...
        options.append(PhiroHelper.motor(toString: Motor.both))
        options.append(PhiroHelper.motor(toString: Motor.right))
        options.append(PhiroHelper.motor(toString: Motor.left))
        values = options
