...
class BrickCellSoundData: iOSCombobox, BrickCellDataProtocol, iOSComboboxDelegate {
    ...
    required init(frame: CGRect, andBrickCell brickCell: BrickCell?, andLineNumber line: Int, andParameterNumber parameter: Int) {
        ...
        if !(brickCell?.isInserting ?? false) {
            ...
            if brickCell?.scriptOrBrick is BrickSoundProtocol != nil {
                let soundBrick = brickCell?.scriptOrBrick as? (Brick & BrickSoundProtocol)
                let currentSound: Sound? = soundBrick?.sound(forLineNumber: line, andParameterNumber: parameter)
                for sound: Sound? in (soundBrick?.script!.object!.soundList)! {
                    if let aName = sound?.name {
                ...
                if let aName = currentSound?.name {
                    if currentSound != nil && !options.contains(aName) {
                        options.append(aName)
                        currentOptionIndex = optionIndex
                    }
                }
